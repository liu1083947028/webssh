...
jQuery(function($){
  ...
  function ajax_complete_callback(resp) {
    ...
    if (!msg.id) {
      log_status(msg.status + ': ' + msg.statusText, true);
      state = DISCONNECTED;
      return;
    }
    ...
    var ws_url = 'ws://' + window.location.host,
        ...
        url = ws_url + join + 'shell?id=' + msg.id,
    ...
    term.on('data', function(data) {
      // console.log(data);
      sock.send(JSON.stringify({'data': data, clientId: localStorage.getItem("clientId")}));
    });
  ...
  function connect_without_options() {
    ...
    var form = document.querySelector(form_id),
        ...
        url = form.action + '?clientId=' + (localStorage.getItem("clientId") || ""),
        data, pk;
